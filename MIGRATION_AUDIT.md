...
- **Target repository:** this repo (`brian10420/claude_research_team`, local
  `~/Brian/claude-research-team`), to be renamed **`research-os`** (owner `brian10420`).
...
1. **Target repo:** `~/Brian/claude-research-team`. EchoMind (`~/Brian/Echo_mind`) is
...
The migration session was launched in `~/Brian/Echo_mind` (the EchoMind research
codebase), but the mission describes a *skill collection* that gets renamed, holds
cross-project rules, and ships templates "to copy into EchoMind". That is this repo:
the public mirror of EchoMind's private `.claude/` team (MIT; `main` @ `9775931`;
`sync-status` up to date with the private originals on 2026-09-04).

Why it cannot be EchoMind: (a) "GitHub repo rename to research-os" is meaningless for
the research repo; (b) Phase 2 generates `CLAUDE.md` at repo root, which in EchoMind
would overwrite the project-instructions file protected by the standing rule "no agent
edits it"; (c) the manual action "copy templates/project-state into EchoMind".
...
### A. Target repo `~/Brian/claude-research-team` (becomes `research-os`)
...
| A20 | `templates/leakcheck.template.sh` | two-track hygiene pattern | core-methodology | keep | the real deny-list stays private in EchoMind (B8) |
...
### B. EchoMind `~/Brian/Echo_mind` (private canonical source + first consumer; read-only this session)
...
| B2 | `CLAUDE.md` (project instructions, 8.3 KB) | Claude project instructions; protected by the standing rule "no agent edits it" (verified: the ARS PreToolUse guard deliberately does **not** protect `CLAUDE.md` (ARS #459) and only fences ARS's own Bucket-A agents, so the rule is the only guard) | core-methodology (project-level) — stays | untouched; **not** generated by `sync.py` | would have been clobbered had the target been EchoMind (Conflict 1, ruled); a dedicated write-guard is listed as a follow-up in `docs/hardening.md` (Phase 3) |
...
| B7 | `docs/` (100 files: pre-registrations, handoffs, skeletons, sweeps, presentations, release/) | per-project research records | core-methodology content, **per-project** | stays; future distillation into EchoMind's project-state files as reviewed diffs (principle 7) | never bulk-imported |
...
| B12 | Obsidian vault `~/Brian/EchoMind-Vault` (Study Log, Results Ledger, Presentation Log) | per-project truth layer (principle 1) | core-methodology, per-project | referenced via placeholders in templates | live symlink `Claude Memory/` → memory dir: never rename memory files |
...
4. **Two-track doctrine inversion** — RULED: recorded as a consequence inside
   ...
   After: `research-os/core/` is canonical for cross-project rules and roles; EchoMind's
...
5. **Model pins.** Standing rule: the Fable pin was reversed on 2026-07-09 — nothing may
...
9. **Public repo hygiene.** Everything written into `core/` must be project-neutral
   ...
   The private leak-gate (`~/Brian/Echo_mind/tools/release/leakcheck_public_tree.sh`) is
   run against the working tree before the final report and its result printed.

## Execution outline (all in `~/Brian/claude-research-team`, one commit per phase)
...
- **Phase 1**: `core/` (SCIENTIFIC_RULES + 8 state files, schema headers + one example
  ...
  (doctrine), the private EchoMind files read-only for cross-check.
...
- **Phase 3**: canary protocol text into SCIENTIFIC_RULES, both adapters, every role
  ...
  EchoMind's `CLAUDE.md`). → `phase-3: canary handshake`.
...
- **Phase 6**: `eval/RUBRIC.md` (weights 20/30/20/20/10, anchors 1–5, freeze clause),
  ...
  `tasks/T1–T5.md` with EchoMind path placeholders. → `phase-6: evaluation scaffold`.

## Legacy moves (appended in Phase 4)

_(to be filled in Phase 4: one line per moved item with the reason)_
